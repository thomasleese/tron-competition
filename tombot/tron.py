...
class Socket:
    def __init__(self, fd=None, socket=None):
        self.fd = fd
        self.socket = socket

        if socket:
            self.log_fd = open("logs/{0}.txt".format(time.time()), "a")
    ...
    def mainloop(self, callback):
        if self.fd:
            for line in map(str.strip, self.fd):
                self.read_message(line.split(" "), callback)
        else:
            for line in linesplit(self.socket):
                self.read_message(line.split(" "), callback)

    def read_message(self, msg, callback):
        if self.log_fd:
            self.log_fd.write(" ".join(msg) + "\n")
            self.log_fd.flush()
